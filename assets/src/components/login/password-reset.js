...
import { Form, Button, Modal, Container, Col } from 'react-bootstrap';
import { observable } from 'mobx';
import { observer } from "mobx-react";
import { Spinner } from "@blueprintjs/core";
import { handleResetPassword, handleSendPasswordNew, handlePassWordCodigoValid } from "../../stores/login/api"
import App from '../../App';
...
class ResetPassword extends React.Component {
    ...
    async handleSendCodigo() {
        ...
        if (this.obs.email === "") {
            ...
            if (res.data.exist) {
                ...
                AppToaster.show({ message: "Código enviado para o email", intent: "success" });


            } else {

                AppToaster.show({ message: "Este email não existe cadastrado em nossa base de dados", intent: "danger" });
    ...
    async handleSendNewPassword() {
        ...
        if (res.data.ok) {
            ...
            AppToaster.show({ message: "Senha alterada com sucesso", intent: "success" });

        } else {

            AppToaster.show({ message: "Não foi possével alterar a senha, tente novamente mais tarde", intent: "danger" })
                ...
                {!this.obs.is_open_codigo ?
                    ...
                    <>
                        ...
                        <Form.Group as={Col}>
                            ...
                            <Form.Control id="nome" type="text" onChange={(el) => this.obs.email = el.target.value} placeholder="Entre com seu email" />

                        </Form.Group>

                        <Form.Group as={Col}>
                            <Button onClick={() => this.handleSendCodigo()} variant="success" type="button">
                                Enviar Código
                            </Button>
                        </Form.Group>
                    ...
                    this.obs.valid_codigo ?
                        ...
                        <>
                            <Form.Group as={Col}>
                                ...
                                <Form.Control type="password" value={this.obs.new_password} onChange={(el) => this.obs.new_password = el.target.value} placeholder="Entre com sua nova senha" />

                            </Form.Group>

                            <Form.Group as={Col}>
                                <Button onClick={() => this.handleSendNewPassword()} variant="success" type="button">
                                    Alterar Senha
                                </Button>
                            </Form.Group>
                        ...
                        <>
                            <Form.Group as={Col}>
                                ...
                                <Form.Control id="nome" type="text" value={this.obs.codigo_front} onChange={(el) => this.obs.codigo_front = el.target.value} placeholder="Entre com seu código" />

                            </Form.Group>

                            <Form.Group as={Col}>
                                <Button onClick={() => this.handleValidaCodigo()} variant="success" type="button">
                                    Validar código
                                </Button>
                            </Form.Group>
